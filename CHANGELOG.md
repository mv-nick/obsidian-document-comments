...
- **The card you point at comes into view, and nothing moves otherwise.** On a heavily annotated note the margin stack could run past the end of the document, leaving the last cards unreachable. Now, when you hover highlighted text whose card is not fully visible, that comment becomes the pivot of the stack: its card sits exactly beside its text and the other cards move up or down out of its way, and it keeps that place until another out-of-view comment is hovered, so it doesn't jump away while you move the mouse to it. If the card still doesn't fit (a tall card near the bottom edge), the note scrolls the minimum needed to show the whole card, in Live Preview and Reading view. A card that is already fully visible is never moved, and hovering cards in the column only highlights them; hover events the scroll itself re-dispatches are ignored until the pointer moves, so a reveal can't cascade or undo itself.
